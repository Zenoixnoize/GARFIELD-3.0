...
Asena.addCommand({pattern: 'need ?(.*)', fromMe: false, desc: Lang.GET_DESC}, (async (message, match) => { 
        ...
        if (!match[1].includes('mp3') && match[1].includes('youtu.be') || match[1].includes('youtube.com')) {
            var VID = '';
            try {
                if (match[1].includes('youtube')) {
                    var tsts = match[1].replace('youtube5s', '')
            ...
            var yt = ytdl(VID, {filter: format => format.container === 'mp3' && ['128kbps'].map(() => true)});
            ...
            yt.on('end', async () => {
                ...
                await message.client.sendMessage(message.jid,fs.readFileSync('./' + VID + '.mp3'), MessageType.video, {quoted: message.data ,mimetype: Mimetype.mp4});
            });
    }
